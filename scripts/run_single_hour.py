...
def _save_column_event_scan(
    ...
    *,
    ...
    scan_df_plot = scan_df[~scan_df["proc_label"].isin(["unknown", "no_data"])].copy()
    ...
    scan_df_events = scan_df_events[
        ~scan_df_events["proc_label"].isin(["steady_or_weak", "unknown", "no_data"])
    ].copy()
...
def main() -> None:
    ...
    try:
        ...
        if args.force_reprocess or not product_path.exists():
